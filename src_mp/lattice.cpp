...
#pragma omp parallel for
	for (auto it = complementOfClosure.begin(); it != complementOfClosure.end();
		 it++) {
		double sum = 0;

		list<pair<long int, long int>> neighbours =
			getNeighboursIndicesOf(it->first, it->second);
		...
		for (auto it = boundary.begin(); it != boundary.end(); it++) {
			...
			for (auto& neighbour : neighbours) {
				if ((*this)[neighbour.first][neighbour.second]
					...
					sum += (*this)[neighbour.first][neighbour.second]
							   .vaporMass_before;
			}
			sum += (7 - neighbours.size()) *
				   (*this)[it->first][it->second].vaporMass_before;
...
#pragma omp parallel for
	for (auto& indices : boundary) {
		...
		list<pair<long int, long int>> neighbours =
			getNeighboursIndicesOf(indices.first, indices.second);
		neighbours.pop_front();	 // we exclude the cell itself

		size_t numberOfNeighboursInSnowflake = 0;
		double sumOfVaporMass = cell.vaporMass_before;
		for (auto& neighbour : neighbours) {
			if ((*this)[neighbour.first][neighbour.second].inSnowflake_before)
		...
		bool condition1 = (numberOfNeighboursInSnowflake == 1 ||
		...
		bool condition3 = numberOfNeighboursInSnowflake >= 4;
...
#pragma omp parallel for
	for (auto indices = complementOfClosure.begin();
		 indices != complementOfClosure.end(); indices++) {
		Cell& cell = (*this)[indices->first][indices->second];
	...
	if (!boundary.empty()) {
#pragma omp parallel for
		for (auto indices = boundary.begin(); indices != boundary.end();
			 indices++) {
			Cell& cell = (*this)[indices->first][indices->second];
